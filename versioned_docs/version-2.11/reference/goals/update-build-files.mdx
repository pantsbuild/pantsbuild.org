...
<Option
  ...
  toml_repr={`[update-build-files]
fix_python_macros = <bool>`}
  default_repr={`False`}
>

Update references to targets generated from `python_requirements` and `poetry_requirements` from the old deprecated macro mechanism to the new target generation mechanism described at https://www.pantsbuild.org/v2.11/docs/targets#target-generation.
