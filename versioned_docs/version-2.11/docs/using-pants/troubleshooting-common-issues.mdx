...
- Ambiguity. >1 target exposes the same module/package.
  - If it's a third-party dependency, you should likely use multiple "resolves" (lockfiles). Each resolve should have no more than one of the same requirement. See [Python](../python/python-overview/third-party-dependencies.mdx#multiple-lockfiles) and [JVM](../java-and-scala/index.mdx).
...
- Multiple resolves (Python and JVM).
  ...
  - See [Python](../python/python-overview/third-party-dependencies.mdx#multiple-lockfiles) and [JVM](../java-and-scala/index.mdx).
