...
Set to the string `<default>` to use a lockfile provided by Pants, so long as you have not changed the `--version` option. See https://github.com/pantsbuild/pants/blob/release_2.21.2rc0/src/python/pants/jvm/test/junit.default.lockfile.txt for the default lockfile contents.
