...
For many [plugin tasks](../common-plugin-tasks/index.mdx), you will be extending pre-existing goals, such as adding a new linter to the `lint` goal. However, you may instead want to create a new goal, such as a `publish` goal. This page explains how to create a new goal.

As explained in [Concepts](./concepts.mdx), `@goal_rule`s are the entry-point into the rule graph. When a user runs `./pants my-goal`, the Pants engine will look for the respective `@goal_rule`. That `@goal_rule` will usually request other types, either as parameters in the `@goal_rule` signature or through `await Get`.
...
There are four steps to creating a new [goal](../../using-pants/concepts/goals.mdx) with Pants:

1. Define a subclass of `GoalSubsystem`. This is the API to your goal.
   1. Give a description in the docstring.
   2. Set the class property `name` to the name of your goal.
   3. You may register options through the class method `register_options()`. See [Options and subsystems](./options-and-subsystems.mdx).
...
You may alternatively request `Addresses` from `pants.engine.addresses` if you do not care about the metadata of the targets. You may also alternatively request `TransitiveTargets` from `pants.engine.target` if you need all of the transitive dependencies included.
...
Instead, request `Targets`, which will give you all of the targets that the user specified on the command line. The engine knows how to resolve this type because it can go from `AddressSpecs` and `FilesystemSpecs` -> `Specs` -> `Addresses` -> `Targets`.
...
If you only care about files, and you don't need any metadata from BUILD files, then you can request `SourcesSnapshot` instead of `Targets`.
...
@goal_rule
async def hello_world(console: Console, sources_snapshot: SourcesSnapshot) -> HelloWorld:
    for f in sources_snapshot.snapshot.files:
