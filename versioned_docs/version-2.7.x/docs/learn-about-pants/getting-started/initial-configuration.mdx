...
By default, Pants recognizes having no source root, or having `src`, `src/python`, or `src/py` as source roots. If your project has a different structure, see [Source roots](../../using-pants/key-concepts/source-roots.mdx) for how to configure them, and for examples of different project structures with Pants.

## 3. Enable Backends

Most Pants functionality is provided via pluggable [_backends_](../../using-pants/key-concepts/backends.mdx). You enable a backend by listing it under the `backend_packages` config key in `pants.toml`.
...
See [Create initial BUILD files](./create-initial-build-files.mdx) for more information about how to do that.
