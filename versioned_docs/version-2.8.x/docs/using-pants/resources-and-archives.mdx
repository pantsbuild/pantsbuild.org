...
A [`resource`](../../reference/targets/resource.mdx) target is for files that are members of code packages, and are loaded via language-specific mechanisms, such as Python's `pkgutil.get_data()` or Java's `getResource()`.
...
[Source root](./key-concepts-to-know-about-pants/source-roots.mdx) stripping applies to resources, just as it does for code. In the example above, Python loads the resource named `project/config`, rather than `src/python/project/config.json`.
...
The `files` field is a list of `file`, `files`, and `relocated_files` targets. See [resources](./resources-and-archives.mdx) for more details.
