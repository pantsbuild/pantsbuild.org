...
You can change IPython's version with `[ipython].version`. If you change it, Pants's default lockfile for IPython will not work. Either set the `lockfile` option to a custom path or `"<none>"` to opt-out. See [Third-party dependencies](../python-overview/third-party-dependencies.mdx#tool-lockfiles).
...
:::danger IPython does not yet work with Pantsd
