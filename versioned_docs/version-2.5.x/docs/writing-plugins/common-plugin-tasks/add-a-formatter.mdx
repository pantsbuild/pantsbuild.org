...
async def format_bash_targets(
    ...
    results = []
    fmt_request_types = union_membership.union_rules[
        BashFmtRequest
    ]
    for fmt_request_type in fmt_request_types:
        ...
        if not request.field_sets:
            continue
        result = await Get(EnrichedFmtResult, BashFmtRequest, request)
...
Then, hook this up to a new subclass of both `LintRequest` _and_ the `FmtRequest` you set up in step 2 for your language, such as `BashFmtRequest` or `PythonFmtRequest`:
