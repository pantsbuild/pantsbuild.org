...
```dockerfile title="src/example/Dockerfile"
...
You may also use any Docker build arguments (when configured as described in [Docker build arguments](./docker-overview.mdx#build-arguments)) for interpolation into the `image_tags` in the corresponding `docker_image`:
...
:::note Generating dynamic tags in a plugin
If you don't want to use the environment variable method described above, you'll need to write some custom plugin code. Don't hesitate to [reach out](/community/getting-help) for help with this.
