...
pants_distdir_legacy_paths = <bool>`}
  ...
  removal_hint={'The pre-2.0 naming scheme for artifacts built with `./pants package` is being removed because it often resulted in ambiguity and overwriting other artifacts. Use the field `output_path` on each `pex_binary`, `python_awslambda`, and `archive` target where you would like to avoid the default.'}
>
