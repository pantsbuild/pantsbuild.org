...
If `./pants dependencies` is behaving how you'd expect, but certain imports of first-party code are still missing, try running with `--no-process-execution-local-cleanup` and inspect the logged chroot to see if the files you're expecting are present. If files are missing, you may need to adjust the [`sources` field](../concepts/targets-and-build-files.mdx#sources) of some of your dependencies.
