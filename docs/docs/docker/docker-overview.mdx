...
BuildKit supports exporting build cache to an external location, making it possible to import in future builds. Cache backends can be configured using the [`cache_to`](../../reference/targets/docker_image.mdx#cache_tocode) and [`cache_from`](../../reference/targets/docker_image.mdx#codecache_from) fields.
