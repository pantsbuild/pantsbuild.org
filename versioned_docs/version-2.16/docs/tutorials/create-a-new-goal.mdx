...
We'll be writing an [in-repo plugin](../writing-plugins/overview.mdx#in-repo-plugins), and expect you to have the `pants-plugins/project_version` directory as well as the `pants.toml` file with this configuration:
...
```python tab={"label":"pants-plugins/project_version/targets.py"}
from pants.engine.target import COMMON_TARGET_FIELDS, SingleSourceField, Target
