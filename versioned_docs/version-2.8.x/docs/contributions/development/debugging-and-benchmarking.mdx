...
When benchmarking, you must decide if you care about cold cache performance vs. warm cache (or both). If cold, use `--no-pantsd --no-process-execution-local-cache`. If warm, use hyperfine's option `--warmup=1`.
