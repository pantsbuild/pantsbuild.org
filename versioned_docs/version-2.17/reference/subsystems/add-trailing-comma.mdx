...
If true, don&#x27;t use add-trailing-comma when running `scie-pants-linux-x86_64 fmt` and `scie-pants-linux-x86_64 lint`.
...
<Option
  ...
  toml_repr={`[add-trailing-comma]
version = <str>`}
  default_repr={`add-trailing-comma>=2.2.3,<3`}
