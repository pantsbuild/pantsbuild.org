...
const estimateSearch = (location) => {
    ...
    const allComponents = strings.map(decodeURI).flatMap(s => [...s.matchAll(componentsRe)].flat());
    const interestingComponents = allComponents.filter(s => !s.match(boringComponents))
      ...
      <div className="row">
        <div className="col col--6 col--offset-3">
          ...
          {location.pathname.startsWith("/v1") ?
            ...
            <span>
              <p>Try a <Link to={`/search?q=${encodeURIComponent(estimatedSearchQuery)}`}>search for '{estimatedSearchQuery}'</Link>?</p>
              <p>
