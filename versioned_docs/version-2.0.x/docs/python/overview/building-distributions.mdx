...
The `pex_binary` target must explicitly set the field `entry_point` in the form `path.to.module:my_func`, e.g. `project.app:main`. See https://python-packaging.readthedocs.io/en/latest/command-line-scripts.html#the-console-scripts-entry-point.
