...
For the first two approaches, Pants will use [dependency inference](../../using-pants/concepts/targets-and-build-files.mdx), which you can confirm by running `./pants dependencies path/to:app`. You can also manually add to the `dependencies` field.
