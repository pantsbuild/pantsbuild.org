...
Alternatively you may provide the Docker build instructions inline in your BUILD file as [`instructions`](../../reference/targets/docker_image.mdx#codeinstructionscode) on your `docker_image` if you don't want to create a `Dockerfile`.
...
When your `Dockerfile` is a multi-stage build file, you may specify which stage to build with the [`--docker-build-target-stage`](../../reference/subsystems/docker.mdx#section-build-target-stage) for all images, or provide a per image setting with the `docker_image` field [`target_stage`](../../reference/targets/docker_image.mdx#codetarget_stagecode).
...
Secrets are supported for `docker_image` targets with the [`secrets`](../../reference/targets/docker_image.mdx#codesecretscode) field. The defined secrets may then be mounted in the `Dockerfile` as [usual](https://docs.docker.com/develop/develop-images/build_enhancements/#new-docker-build-secret-information).
...
See the example for the [`secrets`](../../reference/targets/docker_image.mdx#codesecretscode) field.
:::

### External cache storage backends

BuildKit supports exporting build cache to an external location, making it possible to import in future builds. Cache backends can be configured using the [`cache_to`](../../reference/targets/docker_image.mdx#codecache_tocode) and [`cache_from`](../../reference/targets/docker_image.mdx#codecache_fromcode) fields.
...
```python title="src/docker/hw/BUILD"
...
```dockerfile title="src/docker/hw/Dockerfile"
FROM python:3.8
...
```text title="src/docker/hw/msg.txt"
Hello, Docker!
```

```python title="src/python/hw/BUILD"
python_sources(name="lib")

pex_binary(name="bin", entry_point="main.py")
```

```python title="src/python/hw/main.py"
import os
