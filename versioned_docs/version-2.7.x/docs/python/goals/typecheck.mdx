...
When you run `./pants typecheck ::`, Pants will skip any files belonging to skipped targets.
...
Some MyPy plugins also include type stubs, such as `django-stubs`. For type stubs to be used, they must be loaded like a normal [third-party dependency](../overview/third-party-dependencies.mdx), such as including in a `requirements.txt` file. So, to use `django-stubs`, you must both include it in `[mypy].extra_requirements` and as a dependency of your code.
...
Pants will copy all reports into the folder `dist/typecheck/mypy`.
