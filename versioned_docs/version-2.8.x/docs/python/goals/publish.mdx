...
This will register a new `pypi_repositories` field for the `python_distribution` target, so when you run `./pants publish` for those targets, they will package them and then publish the distributions using Twine to the repositories specified in your BUILD files.

## PyPI Repositories

When publishing a `python_distribution`, you need to tell Pants which repositories to publish to. That is done with a new `pypi_repositories` field on the `python_distribution`.
