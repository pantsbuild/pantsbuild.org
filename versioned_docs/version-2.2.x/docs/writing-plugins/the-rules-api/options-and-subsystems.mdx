...
As explained in [Options](../../using-pants/concepts/options.mdx), options are partitioned into named scopes, like `[test]` and `[isort]`. Each of these scopes corresponds to a _subsystem_.
...
Refer to [Options](../../using-pants/concepts/options.mdx#option-types) for how a user specifies these different types.
