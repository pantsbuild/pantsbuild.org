...
Set to the string `<default>` to use a lockfile provided by Pants, so long as you have not changed the `--version` option. See https://github.com/pantsbuild/pants/blob/release_2.30.0.dev1/src/python/pants/jvm/jar_tool/jar_tool.lock for the default lockfile contents.
