...
## Highlights in [2.28](https://github.com/pantsbuild/pants/blob/2.28.x/docs/notes/2.28.x.md) include:
...
- 🔮 **Polymorphic rules** - The plugin API now supports runtime polymorphism with @rule call-by-name semantics.
...
- 📦 **Batch remote reads** - Experimental support for batch reads from the gRPC Remote Execution API significantly increases throughput.
- 🐍 **Direct type stub generation** - Protobuf can now generate Python type stubs directly via `protoc`.
...
## Pants is an open-source project

The changes are all contributed by our community. If you want to see something more in the next changelog, join us on [GitHub](https://github.com/pantsbuild/pants) and become a [contributor](https://www.pantsbuild.org/stable/docs/contributions).
