...
| `--remote_instance_name` | string | Path to a file containing an authorization token to pass in remote execution requests. Necessary only if the server requires authentication. The token will be placed in the `authorization` header in gRPC requests as `Bearer TOKEN`. |
