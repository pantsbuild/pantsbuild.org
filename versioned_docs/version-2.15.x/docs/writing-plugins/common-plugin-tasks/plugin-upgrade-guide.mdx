...
formatters like `Prettier` running on your codebase _without_ boilerplate targets, as well as Pants
...
Pants 2.15 introduces the concept of ["Target Environments"](../../using-pants/environments:-cross-platform-or-remote-builds.mdx), which allow Pants to execute processes in remote or local containerized environments (using Docker), and to specify configuration values for those environments.
