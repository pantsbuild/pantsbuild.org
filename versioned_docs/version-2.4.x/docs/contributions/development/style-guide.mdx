...
Why? Beyond it being easier to only need to remember one name, Pytest has [magic](https://docs.pytest.org/en/6.2.x/assert.html) on the `assert` statement to show how each of the values was derived.
