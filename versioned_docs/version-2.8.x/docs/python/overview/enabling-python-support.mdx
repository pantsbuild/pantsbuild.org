...
Enable the Python [backend](../../using-pants/key-concepts-to-know-about-pants/backends.mdx) like this:
...
You can generate these targets by running [`./pants tailor`](../../getting-started/getting-started/create-initial-build-files.mdx).
...
However, if you run into issues, you can set the `interpreter_search_paths` option in the `[python]` scope:
