...
When writing stubs for third-party libraries, you may need the set up the `[source].root_patterns` option so that [source roots](../../using-pants/key-concepts-to-know-about-pants/source-roots.mdx) are properly stripped. For example:
