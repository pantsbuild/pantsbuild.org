...
Go to [dash.readme.io/project/pants](https://dash.readme.io/project/pants). In the top left dropdown, where it says the current version, click "Manage versions". Click "Add new version" and use a "v" with the minor release number, e.g. "v1.28". Fork from the prior release.
...
Announce the release to [pants-devel](https://groups.google.com/forum/#!forum/pants-devel) and the #announce channel in Slack.
...
```
Subject: [dev release] pantsbuild.pants 2.6.0.dev0


The first weekly dev release for the 2.6 series is now available [on PyPI](https://pypi.org/project/pantsbuild.pants/2.6.0.dev0/).

See [here](https://github.com/pantsbuild/pants/blob/main/src/python/pants/notes/2.6.x.md) for the changelog.
...
If the patch version's `rc0` has already been released, reply to that email thread for the rest of the patch's release candidates. That is, bundle `2.6.0` release candidates together, and `2.6.1` candidates together, etc.

```
Subject: [release candidate] pantsbuild.pants 2.6.0rc1


The second release candidate for 2.6.0 is now available [on PyPI](https://pypi.org/project/pantsbuild.pants/2.6.0rc1/).
...
```
Subject: [stable release] pantsbuild.pants 2.6.0


The first stable release of the 2.6 series is now available [on PyPI](https://pypi.org/project/pantsbuild.pants/2.6.0/).

See our [blog post](https://blog.pantsbuild.org/introducing-pants-build-2-4-0/) summarizing the release series, and [here](https://github.com/pantsbuild/pants/blob/main/src/python/pants/notes/2.6.x.md) for a more detailed changelog.

Thanks to all of the contributors to the 2.6 series!
