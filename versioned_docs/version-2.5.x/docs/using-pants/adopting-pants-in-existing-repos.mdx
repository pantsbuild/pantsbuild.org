...
First, create a BUILD file in each directory with Python code. As explained in [Targets and BUILD files](./key-concepts/targets-and-build-files.mdx), you can use fewer BUILD files, such as one BUILD file per subproject; but we find that one BUILD file per directory is a good default.
