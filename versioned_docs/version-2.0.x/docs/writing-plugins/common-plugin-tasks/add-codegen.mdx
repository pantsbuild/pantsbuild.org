...
Pants supports code generators that convert a protocol language like Protobuf into other languages, such as Python or Java. The same protocol source may be used to generate multiple different distinct languages.

Pants will not actually write the generated files to disk. Instead, any targets that depend on the protocol targets will cause their code to be generated, and those generated files will be copied over into the "chroot" (temporary directory) where Pants executes.
...
You will need a define a new target type to allow users to provide metadata for their protocol files, e.g. their `.proto` files. See [Creating new targets](../the-target-api/creating-new-targets.mdx) for a guide on how to do this.
...
You should define a subclass of `Sources`, like `ProtobufSources` or `ThriftSources`. This is important for Step 3.

Typically, you will want to register the `Dependencies` field.

### Target type already exists?

If Pants already has a target type for your protocol—such as Pants already having a `ProtobufLibrary` defined—you should not create a new target type.

Instead, you can optionally add any additional fields that you would like through plugin fields. See [Extending pre-existing targets](../the-target-api/extending-pre-existing-targets.mdx).
...
This example hardcodes the injected address. You can instead add logic to your rule to make this dynamic. For example, in Pants's Protobuf implementation, the `python-protobuf` [subsystem](../the-rules-api/options-and-subsystems.mdx) has a `--runtime-dependencies` option. The rule takes that subsystem as a parameter, and then converts the option's string value to an address with `Get(Addresses, UnparsedAddressInputs)`. See [protobuf/python/python_protobuf_subsystem.py](https://github.com/pantsbuild/pants/blob/master/src/python/pants/backend/codegen/protobuf/python/python_protobuf_subsystem.py).
...
class Protoc(ExternalTool):
    ...
    def generate_url(self, plat: Platform) -> str:
        plat_str = "osx" if plat == Platform.darwin else "linux"
        return (
...
`GenerateSourcesRequest` tells Pants the `input` and the `output` of your code generator, such as going from `ProtobufSources -> PythonSources`. Pants will use this to determine when to use your code generation implementation.
...
The `input` should be the `Sources` class for your protocol target from Step 1.

The `output` should typically be the `Sources` class corresponding to the "language" you're generating for, such as `JavaSources` or `PythonSources`. The `output` type will understand subclasses of what you specify, so, generally, you should specify `PythonSources` instead of something more specific like `PythonLibrarySources` or `PythonBinarySources`.

Note that your rule will not actually return an instance of the `output` type, e.g. `PythonSources`. Codegen rules only return a `Snapshot`, rather than a whole `Sources` field. The `output` field is only used as a signal of intent.

Finally, register your new `GenerateSourcesRequest` with a [`UnionRule`](../the-rules-api/union-rules-advanced.mdx).
...
The `GenerateSourcesRequest` parameter will have two fields: `protocol_sources: Snapshot` and `protocol_target: Target`. Often, you will want to include `protoc_sources` in the `input_digest` to the `Process` you use to run the generator. You can use `protocol_target` to look up more information about the input target, such as finding its dependencies.

The rule should return `GeneratedSources`, which take a [`Snapshot`](../the-rules-api/file-system.mdx) as its only argument. This should be a Snapshot of the generated files for the input target.
...
For many code generators, you will need to get the input target's direct or transitive dependencies and include their sources in the `input_digest`. See [Rules and the Target API](../the-rules-api/rules-and-the-target-api.mdx).

:::danger Use `DependenciesRequestLite` and `TransitiveTargetsRequestLite`
For codegen, you will get a rule graph error if you use `DependenciesRequest` and `TransitiveTargetsRequest` like normal. Instead, use `DependenciesRequestLite` and `TransitiveTargetsRequestLite`, both from `pants.engine.target`.

These behave almost identically and take the same arguments in their constructors. However, these "lite" versions do not include dependency inference in the result. Unless you are generating code with `PythonSources` as the input, this should be fine.

https://github.com/pantsbuild/pants/issues/10917 tracks this bug.
:::

You will likely need to add logic for handling [source roots](../../using-pants/concepts/source-roots.mdx). For example, the code generator may not understand source roots so you may need to [strip source roots](../the-rules-api/rules-and-the-target-api.mdx) before putting the sources in the `input_digest`. Likely, you will want to restore a source root after generation because most Pants code will assume that there is a source root present. In the below example, we restore the original source root, e.g. `src/protobuf/f.proto` becomes `src/protobuf/f_pb2.py`. See [`protobuf/python/rules.py`](https://github.com/pantsbuild/pants/tree/master/src/python/pants/backend/codegen/protobuf/python/rules.py) for a more complex example that allows the user to specify what source root to use through a field on the `protobuf_library`.
...
:::note Tip: testing out if your implementation is working
Because codegen is only triggered when call sites specifically request it, there is no quick way to verify that your new rule works correctly.

It may help to add a temporary [`@goal_rule`](../the-rules-api/goal-rules.mdx) when iterating, like this:
...
class TestCodegenSubsystem(GoalSubsystem):
    name = "codegen"


class TestCodegen(Goal):
    subsystem_cls = TestCodegenSubsystem


@goal_rule
async def test_codegen(targets: Targets, workspace: Workspace) -> TestCodegen:
    ...
    workspace.write_digest(generated_sources.snapshot.digest, path_prefix="codegen")
    return TestCodegen(exit_code=0)
...
Replace `for_sources_types` with the `Sources` type your implementation generates.

Then, update your plugin's `register.py` to register this new rule, and run `./pants codegen path/to/protocol_file.ext`.
:::

## 5. Audit call sites to ensure they've enabled codegen

Call sites must opt into using codegen, and they must also specify what types of sources they're expecting. See [Rules and the Target API](../the-rules-api/rules-and-the-target-api.mdx) about the `Sources` field.

For example, if you added a code generator that goes from `ProtobufSources -> JavaSources`, then Pants's Python backend would not use your new implementation because it ignores `JavaSources`.
...
It may also be helpful to inspect the code for the rest of your plugin and/or Pants's own rules that you're using. Check that `HydrateSourcesRequest` and `SourceFilesRequest` have set `enable_codegen=True` and that they put the correct value for the argument `for_sources_type`.
