...
<Option
  cli_repr={`--download-terraform-args="[<shell_str>, <shell_str>, ...]", ... -- [<shell_str> [<shell_str> [...]]]`}
  env_repr='PANTS_DOWNLOAD_TERRAFORM_ARGS'
  default_repr={`[]`}
>

Arguments to pass directly to terraform, e.g. `--download-terraform-args='-auto-approve'`.
...
<Option
  cli_repr={`--download-terraform-extra-env-vars="['<str>', '<str>', ...]"`}
  env_repr='PANTS_DOWNLOAD_TERRAFORM_EXTRA_ENV_VARS'
  default_repr={`[]`}
>
