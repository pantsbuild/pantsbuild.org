...
<Option
  cli_repr={`--helm-k8s-parser-crd="{'key1': val1, 'key2': val2, ...}"`}
  env_repr='PANTS_HELM_K8S_PARSER_CRD'
  toml_repr={`[helm-k8s-parser.crd]
key1 = val1
key2 = val2
...`}
  default_repr={`{}`}
...
Additional custom resource definitions be made available to all Helm processes or during value interpolation. Example:
[helm-k8s-parser.crd]
