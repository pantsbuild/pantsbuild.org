...
```python
# Good
lang = "hola" if lang == "spanish" else "hello"
...
### Use `assert` instead of `self.assertX`
...
def test_demo(self) -> None:
   ...
   self.assertIn("hello", z)
```

Why? Beyond it being easier to only need to remember one name, Pytest has [magic](https://docs.pytest.org/en/latest/assert.html) on the `assert` statement to show how each of the values was derived.

### Use top-level functions when possible

Prefer to write your tests as top-level functions instead of using `unittest.TestCase`.
