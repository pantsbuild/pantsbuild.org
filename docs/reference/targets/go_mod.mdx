...
Generates `go_third_party_package` targets (or `go_third_party_module` targets under `[golang].third_party_target_granularity = "module"`) based on the `require` directives in your `go.mod`.
