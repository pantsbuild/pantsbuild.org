...
If you want the same requirement to show up in multiple resolves, use the [`parametrize`](../../using-pants/key-concepts/targets-and-build-files.mdx) mechanism.
...
If a first-party target is compatible with multiple resolves—such as some utility code—you can either use the [`parametrize` mechanism](../../using-pants/key-concepts/targets-and-build-files.mdx) with the `resolve` field or create distinct targets for the same entity.
...
:::note Incremental migration from Poetry to Pex
...
:::caution Local file requirements do not yet work with lockfiles
...
Indexes are assumed to have a nested structure (like [http://pypi.org/simple](http://pypi.org/simple)), whereas repos are flat lists of packages.
...
You can use [config file `%(env.ENV_VAR)s` interpolation](../../using-pants/key-concepts/options.mdx#config-file-interpolation)
...
[.pants.rc file](../../using-pants/key-concepts/options.mdx#pantsrc-file) in each user's Pants repo, that sets this config for
the user:
