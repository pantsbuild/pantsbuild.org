...
- Pants does not know about some of your project files because you do not have targets and BUILD files describing them.
  - Tip: run [`./pants tailor`](../getting-started/create-initial-build-files.mdx), which will generate targets for files without owners. Note that this will not add every target type, such as [`resources` and `files`](./assets-and-archives.mdx).
