...
Pants will then use [dependency inference](../../using-pants/key-concepts/targets-and-build-files.mdx) by looking at your Python import statements to automatically add those targets to the `dependencies` field. For example:
...
If you define a lockfile, Pants will also optimize to avoid resolving requirements more than one time for your project. This greatly speeds up the performance of goals like `test`, `run`, `package`, and `repl`. (See [python-setup](../../../reference/subsystems/python-setup.mdx) for more information on the `resolve_all_constraints` option.)
...
Pants supports the use of a [`pyproject.toml`](https://python-poetry.org/docs/pyproject/) file from a Poetry project to specify third-party dependencies. This includes the use of the caret (`^`) and tilde (`~`) operators, along with all other [Poetry-permitted dependency specifiers](https://python-poetry.org/docs/pyproject/#dependencies-and-dev-dependencies). Note that Poetry headers outside of dependency specification are permissible, but have no value.
