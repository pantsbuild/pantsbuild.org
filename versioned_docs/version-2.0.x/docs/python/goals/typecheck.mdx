...
Add `mypy_plugin = true` to the `[python-protobuf]` scope. See [Protobuf](../../codegen/protobuf-and-grpc.mdx) for more information.
:::

### Add a first-party plugin

To add a [MyPy plugin](https://mypy.readthedocs.io/en/stable/extending_mypy.html) you wrote, define a `mypy_source_plugin` target with the plugin's Python file(s) included in the `sources` field.
