...
Only one `workspace_environment` may be defined in `[environments-preview].names` per platform, and when `{LOCAL_WORKSPACE_ENVIRONMENT_MATCHER}` is specified as the environment, the `workspace_environment` that matches the current platform (if defined) will be selected.
