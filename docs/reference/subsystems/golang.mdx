...
### `third_party_target_granularity`

<Option
  cli_repr={`--golang-third-party-target-granularity=<ThirdPartyTargetGranularity>`}
  env_repr='PANTS_GOLANG_THIRD_PARTY_TARGET_GRANULARITY'
  toml_repr={`[golang]
third_party_target_granularity = <ThirdPartyTargetGranularity>`}
  one_of='package, module'
  default_repr={`package`}
>

The granularity at which third-party targets are generated from a `go_mod` target.

`package`: one `go_third_party_package` target per third-party Go package (the historical behavior).

`module`: one `go_third_party_module` target per third-party Go module. First-party targets depend on the modules they import; `go.mod`/`go.sum` owns transitive resolution. Use `go_binary`&#x27;s `main_import_path` field to build a specific third-party package.

</Option>

### `use_prebuilt_stdlib_archives`

<Option
  cli_repr={`--[no-]golang-use-prebuilt-stdlib-archives`}
  env_repr='PANTS_GOLANG_USE_PREBUILT_STDLIB_ARCHIVES'
  toml_repr={`[golang]
use_prebuilt_stdlib_archives = <bool>`}
  default_repr={`True`}
>

If true, pre-compile the entire Go standard library once per Go toolchain configuration (via `go install std`) and link against those archives, instead of compiling each standard library package as a separate build action.

This is usually significantly faster. Pants automatically falls back to compiling the standard library from source for build configurations that are incompatible with the pre-built archives, such as when using the race detector or other sanitizers, code coverage, or custom compiler/assembler flags.
