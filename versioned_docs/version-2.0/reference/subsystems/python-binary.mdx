...
Config section: <span className="color--primary">`[python-binary]`</span>
...
<Option
  cli_repr={`--[no-]python-binary-emit-warnings`}
  env_repr='PANTS_PYTHON_BINARY_EMIT_WARNINGS'
  toml_repr={`[python-binary]
emit_warnings = <bool>`}
  default_repr={`True`}
>
...
<Option
  cli_repr={`--[no-]python-binary-pex-emit-warnings`}
  env_repr='PANTS_PYTHON_BINARY_PEX_EMIT_WARNINGS'
  toml_repr={`[python-binary]
