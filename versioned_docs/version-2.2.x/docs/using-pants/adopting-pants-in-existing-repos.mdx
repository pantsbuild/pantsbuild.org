...
First, create a BUILD file in each directory with Python code. As explained in [Targets and BUILD files](./concepts/targets-and-build-files.mdx), you can use fewer BUILD files, such as one BUILD file per subproject: but if using coarser grained BUILD files, make sure that all Python source code is included in some target's `sources` parameter (either by explicitly selecting files or with `sources=["**/*.py"]`). We find that using one BUILD file per directory is a good default.
