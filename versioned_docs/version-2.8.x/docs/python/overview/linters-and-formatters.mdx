...
Linter/formatter support is implemented in separate [backends](../../using-pants/key-concepts-to-know-about-pants/backends.mdx) so that they are easy to opt in to individually:
| Backend | Tool |
| :--- | :--- |
...
You can also skip for certain targets with the `skip_tool` field, which can be useful for [incrementally adopting new tools](https://www.youtube.com/watch?v=BOhcdRsmv0s). For example:
...
See [`[pylint].source_plugins`](../../../reference/subsystems/pylint.mdx#section-source-plugins) for instructions to add plugins written by you.
