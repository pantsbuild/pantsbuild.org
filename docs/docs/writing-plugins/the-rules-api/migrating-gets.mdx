...
| `Get(Digest, AddPrefix(...))` | `add_prefix(AddPrefix(...))` | `pants.engine.intrinsics` | `Digest` ⚠️ |
| `Get(Digest, RemovePrefix(...))` | `remove_prefix(RemovePrefix(...))` | `pants.engine.intrinsics` | `Digest` ⚠️ |
...
**Before:**
```python
prefixed_snapshot = await Get(Snapshot, AddPrefix(digest, "prefix/"))
...
prefixed_snapshot = await digest_to_snapshot(prefixed_digest)  # Explicit conversion
