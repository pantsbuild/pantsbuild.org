...
If you notice you cannot publish images with this setup, run the publish command with the PANT_LEVEL=debug environment variable to see what environment variables are actually making it to the docker push command.

# Errors to help Google users

If you are recieving errors like this, it indicates you are attempting to run a linux/x64 binary on linux/arm64 and need to adjust your docker image by updating the build platform as shown in the example:
