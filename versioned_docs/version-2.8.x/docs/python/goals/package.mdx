...
`file` and `files` targets will not be included in the built PEX because filesystem APIs like `open()` would not load them as expected. Instead, use the `resource` and `resources` target or wrap your `pex_binary` in an `archive` target. See [Resources and archives](../../using-pants/resources-and-archives.mdx) for further explanation.
:::
...
We can also build the same Pex by using the address of the `pex_binary` target, as described [here](../../using-pants/key-concepts-to-know-about-pants/targets-and-build-files.mdx).
...
See [Resources and archives](../../using-pants/resources-and-archives.mdx) for how to create a zip or tar file with built binaries and/or loose files in it by using the `archive` target.
