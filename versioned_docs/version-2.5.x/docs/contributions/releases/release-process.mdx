...
```bash
$ PANTS_PEX_RELEASE=STABLE ./build-support/bin/release.sh -p
```
...
The first weekly dev release for the 2.3 series is now available [on PyPI](https://pypi.org/project/pantsbuild.pants/2.3.0.dev0/).

See [here](https://github.com/pantsbuild/pants/blob/main/src/python/pants/notes/2.3.x.md) for the changelog.
...
If the patch version's `rc0` has already been released, reply to that email thread for the rest of the patch's release candidates. That is, bundle `2.3.0` release candidates together, and `2.3.1` candidates together, etc.
...
The first stable release of the 2.3 series is now available [on PyPI](https://pypi.org/project/pantsbuild.pants/2.3.0/).

See our [blog post](https://blog.pantsbuild.org/introducing-pants-build-2-4-0/) summarizing the release series, and [here](https://github.com/pantsbuild/pants/blob/main/src/python/pants/notes/2.3.x.md) for a more detailed changelog.

Thanks to all of the contributors to the 2.3 series!
