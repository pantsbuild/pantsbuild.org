...
```bash
./pants --no-process-execution-local-cleanup lint src/project/app.py
...
...
Because Pants runs processes in hermetic sandboxes (temporary directories), Pants must properly know about your [dependencies](../key-concepts-to-know-about-pants/targets-and-build-files.mdx#dependencies-and-dependency-inference) to avoid import errors.
...
- Pants does not know about some of your project files because you do not have targets and BUILD files describing them.
  - Tip: run [`./pants tailor`](../../../reference/goals/tailor.mdx), which will generate targets for files without owners. Note that this will only teach Pants about your first-party code, but not third-party requirements, which need to be manually added.
- [Source roots](../key-concepts-to-know-about-pants/source-roots.mdx) are not set up properly.
...
- Some targets can never be inferred and must always be explicitly added via the `dependencies` field, e.g. [`file` / `files` and `resource` / `resources` targets](../resources-and-archives.mdx).
- For Python third-party requirements, check if they have any [undeclared dependencies](../../python/overview/third-party-dependencies.mdx#advanced-usage).

When debugging, run [`./pants dependencies path/to/file.ext` and `./pants dependencies --transitive`](../project-introspection.mdx) to see what Pants thinks a particular file depends on. If the dependency you want is not showing up, try temporarily [adding the dependency explicitly](../key-concepts-to-know-about-pants/targets-and-build-files.mdx#dependencies-and-dependency-inference) to see if it fixes the issue; if it does, you can then try to figure out why dependency inference was not working, or simply use the explicit dependency.

If `./pants dependencies` is behaving how you'd expect, but certain imports of first-party code are still missing, try running with `--no-process-execution-local-cleanup` and inspect the logged sandbox to see if the files you're expecting are present. If files are missing, you may need to adjust the [`sources` field](../key-concepts-to-know-about-pants/targets-and-build-files.mdx#sources) of some of your dependencies.
