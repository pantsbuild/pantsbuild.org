...
Sometimes, you may want config specific to your CI, such as turning on test coverage reports. If you want CI-specific config, create a dedicated `pants.ci.toml` [config file](./key-concepts-to-know-about-pants/options.mdx). For example:
...
- [`[python].resolver_num_jobs`](../../reference/subsystems/python-setup.mdx#section-resolver-jobs): maximum number of concurrent jobs to build Python wheels with.
