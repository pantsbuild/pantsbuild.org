...
When defining a `PexRequest` for a tool, you must give arguments for `output_filename`, `requirements`, `main`, and usually `interpreter_constraints`. There are several other optional parameters that may be helpful.
...
Often, you will want to create a [`Subsystem`](./options-and-subsystems.mdx) for your Python tool to allow users to set options to configure the tool. You can subclass `PythonToolBase`, which subclasses `Subsystem`, to do this:
