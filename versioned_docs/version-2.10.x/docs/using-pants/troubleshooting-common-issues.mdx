...
- Pants does not know about some of your project files because you do not have targets and BUILD files describing them.
  - Tip: run [`./pants tailor`](../getting-started/getting-started/create-initial-build-files.mdx), which will generate targets for files without owners. Note that this will not add every target type, such as [`resources` and `files`](./assets-and-archives.mdx).
- [Source roots](./key-concepts/source-roots.mdx) are not set up properly.
...
- \>1 target exports the same module, which means that Pants cannot safely infer which you want to use.
  - For third-party requirements, this usually happens when you have multiple targets referring to the same requirement and which belong to the same "resolve". It's valid to have conflicting versions of the same requirement, but they should belong to distinct resolves. See [Third-party dependencies](../python/overview/third-party-dependencies.mdx) for Python.
...
- Some targets can never be inferred and must always be explicitly added via the `dependencies` field, e.g. [`file` / `files` and `resource` / `resources` targets](./assets-and-archives.mdx).
...
When debugging, run [`./pants dependencies path/to/file.ext` and `./pants dependencies --transitive`](./project-introspection.mdx) to see what Pants thinks a particular file depends on. If the dependency you want is not showing up, try temporarily [adding the dependency explicitly](./key-concepts/targets-and-build-files.mdx#dependencies-and-dependency-inference) to see if it fixes the issue; if it does, you can then try to figure out why dependency inference was not working, or simply use the explicit dependency.

If `./pants dependencies` is behaving how you'd expect, but certain imports of first-party code are still missing, try running with `--no-process-cleanup` and inspect the logged sandbox to see if the files you're expecting are present. If files are missing, you may need to adjust the [`sources` field](./key-concepts/targets-and-build-files.mdx#sources) of some of your dependencies.
