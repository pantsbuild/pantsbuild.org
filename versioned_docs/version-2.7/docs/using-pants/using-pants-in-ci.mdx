...
:::note Examples
See the example-python repository for an [example .travis.yml](https://github.com/pantsbuild/example-python/blob/main/.travis.yml) and [example GitHub Actions worfklow](https://github.com/pantsbuild/example-python/blob/main/.github/workflows/pants.yaml).
:::
...
See [Remote Caching](./remote-caching-and-execution/remote-caching.mdx) for more information.
:::
...
- [`[python-setup].resolver_num_jobs`](../../reference/subsystems/python-setup.mdx#section-resolver-jobs): maximum number of concurrent jobs to build Python wheels with.
