---
title: pydocstyle
description: A tool for checking compliance with Python docstring conventions (http://www.pydocstyle.org/en/stable/).
---
...
Please note that even though `pydocstyle` keeps looking for a configuration file up the directory tree until one is found, Pants will only search for the config files in the repository root (from where you would normally run the `<function bin_name at 0x7f07e2a6f5e0>` command).
...
<Option
  cli_repr={`--pydocstyle-version=<str>`}
  env_repr='PANTS_PYDOCSTYLE_VERSION'
  toml_repr={`[pydocstyle]
version = <str>`}
  default_repr={`pydocstyle[toml]>=6.1.1,<7.0`}
...
<Option
  cli_repr={`--pydocstyle-extra-requirements="['<str>', '<str>', ...]"`}
  env_repr='PANTS_PYDOCSTYLE_EXTRA_REQUIREMENTS'
  toml_repr={`[pydocstyle]
extra_requirements = [
    '<str>',
    '<str>',
...
<Option
  cli_repr={`--[no-]pydocstyle-export`}
  env_repr='PANTS_PYDOCSTYLE_EXPORT'
  toml_repr={`[pydocstyle]
export = <bool>`}
  default_repr={`True`}
  removal_version='2.18.0.dev0'
...
If true, export a virtual environment with Pydocstyle when running `pants export`.
