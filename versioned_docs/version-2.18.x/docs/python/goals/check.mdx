...
See [Lockfiles for tools](../python-overview/lockfiles.mdx#lockfiles-for-tools).
...
If you install MyPy from a [custom lockfile](../python-overview/lockfiles.mdx#lockfiles-for-tools) you can also add type stub requirements to that lockfile.
...
Add any third-party MyPy plugins to a [custom lockfile](../python-overview/lockfiles.mdx#lockfiles-for-tools):
...
For these reasons, it's recommended to load any type-stub libraries that require explicit imports as part of your normal [third-party dependencies](../python-overview/third-party-dependencies.mdx). Alternatively, you can set `# pants: no-infer-dep` on the lines of type-stub imports "guarded" by a check of `if TYPE_CHECKING`.
:::
