...
- `interpreter_constraints=[">=3.6"]`: Indicates that this code is compatible with Python 3.6+. Pants will use this metadata when determining which Python interpreter(s) to use.
...
Most targets have a `sources` field, which determines which source files belong to the target.
...
If we want to use the default `interpreter_constraints` value, we could simply write:
