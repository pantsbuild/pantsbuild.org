...
You must also install the distribution `pantsbuild.pants.testutil`. We recommend using the `pants_requirement()` macro to do this, similar to how you may have [installed Pants as a library](../overview.mdx) when writing your plugin. This will read your `pants_version` from `pants.toml` to ensure that you're using the correct distribution.

```python title="pants-plugins/BUILD"
pants_requirement(name="pants.testutil", dist="pantsbuild.pants.testutil")
```

Pants's dependency inference will infer a dependency on this library whenever you import `pants.testutil`. You may also add the dependency explicitly in the `dependencies` field.
...
For example, some Pants rules take the type `PexInterpreterConstraints` as input. `PexInterpreterConstraints` has a factory method `merge_constraint_sets()` that we can test through a normal unit test.
...
def test_timeout_validation() -> None:
    with pytest.raises(InvalidFieldException):
        PythonTestsTimeout(-100, address=Address("demo"))
    with pytest.raises(InvalidFieldException):
        PythonTestsTimeout(0, address=Address("demo"))
    assert PythonTestsTimeout(5, address=Address("demo")).value == 5
```

:::note How to create a `Target` in-memory
For Approaches #1 and #2, you will often want to pass a `Target` instance to your test, such as a `PythonLibrary` instance.
...
tgt = PythonLibrary(
    ...
    address=Address("project/app", target_name="tgt"),
)
```
...
For Approach #3, you should instead use `rule_runner.add_to_build_file()`, followed by `rule_runner.get_target()`, as this will actually write to a BUILD file and ensure that your rules properly can resolve the target.
...
`run_rule_with_mocks()` will run your rule's logic, but with each argument to your `@rule` provided explicitly by you and with mocks for any `await Get`s. This means that the test will have zero side-effects; for example, `run_rule_with_mocks()` will not actually run a `Process`, nor will it use the file system operations. This is useful when you want to test the inlined logic in your rule, but usually, you will want to use Approach #3.

To use `run_rule_with_mocks`, pass the `@rule` as it's first arg, then `rule_args=[arg1, arg2, ...]` in the same order as the arguments to the `@rule`.

If your `@rule` has any `await Get`s, set the argument `mock_gets=[]` with a `MockGet` object corresponding to each `Get`. A `MockGet` takes three arguments: `output_type: Type`, `input_type: Type`, and `mock: Callable[[OutputType], InputType]`, which is a function that takes an instance of the `input_type` and returns an instance of the `output_type`.
...
def test_find_needle_in_haystack() -> None:
    tgt1 = MockTarget({}, address=Address("", target_name="t1")
    tgt2 = MockTarget({}, address=Address("", target_name="t2")
    tgt3 = MockTarget({}, address=Address("", target_name="t3")
...
If your rule takes `Console` as an argument, you can use `MockConsole` from `pants.testutil.rule_runner`. After running your rule, call `mock_console.stdout.get_value()` and `console.stderr.get_value()` to capture what was written in the rule.

If your rule takes `Workspace` or `InteractiveRunner` as arguments, first create a `pants.testutil.rule_runner.RuleRunner()` instance in your individual test. Then, create a `Workspace` object with `Workspace(rule_runner.scheduler)` or an `InteractiveRunner` instance with `InteractiveRunner(rule_runner.scheduler)`.
...
This means that you must register the rules you directly wrote, and also any rules that they depend on. Pants will automatically register some core rules for you, but leaves off most of them for better isolation of tests. If you're missing some rules, the rule graph will fail to be built with an error message explaining what is missing.
...
def test_example3(second_rule_runner: RuleRunner) -> None:
    second_rule_runner.create_file(..)
    ...
```

### Setting up the content and BUILD files

For most tests, you'll want to create files and BUILD files in your temporary build root. Use `rule_runner.create_file(relpath: str, contents: str)` and `rule_runner.add_to_build_file(relpath: str, target: str)` to do this.
...
def test_example() -> None:
     ...
     rule_runner.add_to_build_file("project", "python_library()\n")
```

These utilities will write the files to the correct location, and also notify the engine that the files were created.

You can also use `rule_runner.create_files()` to create multiple files in the same directory, e.g. `rule_runner.create_files("project", ["f1.txt", "f2.txt"])`.
...
To set options, call `rule_runer.set_options()` with a list of the arguments, e.g. `rule_runner.set_options(["--backend-packages=pants.backend.python", "--pytest-version=pytest>=6.0"])`.

You can also set the keyword argument `env: Dict[str, str]`. If the option starts with `PANTS_`, it will change which options Pants uses. You can include any arbitrary environment variable here; some rules use the parent Pants process to read arbitrary env vars, e.g. the `--test-extra-env-vars` option, so this allows you to mock the environment in your test.
...
def test_find_needle(rule_runner: RuleRunner) -> None:
    ...
    rule_runner.create_files("project", ["f1.txt", "f2.txt", "needle.txt"])
    rule_runner.add_to_build_file(
        "project",
        dedent(
            """\
...
def test_one_target_one_source(rule_runner: RuleRunner) -> None:
    rule_runner.create_file("project/example.ext")
...
`run_pants()` is hermetic by default, meaning that it will not read your `pants.toml`. As a result, you often need to include the option `--backend-packages` in the arguments to `run_pants()`. You can alternatively set the argument `hermetic=False`.
