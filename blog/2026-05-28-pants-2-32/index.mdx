...
- 🔄 Python lockfile generation can use the `sync` option for incremental updates.
...
- Plugins still using the older "call-by-type" style of API will need to [migrate to the call-by-name API](https://www.pantsbuild.org/2.31/docs/writing-plugins/the-rules-api/migrating-gets).
