...
<Option
  cli_repr={`--[no-]python-build-standalone-python-provider-require-inferrable-release-tag`}
  env_repr='PANTS_PYTHON_BUILD_STANDALONE_PYTHON_PROVIDER_REQUIRE_INFERRABLE_RELEASE_TAG'
  toml_repr={`[python-build-standalone-python-provider]
