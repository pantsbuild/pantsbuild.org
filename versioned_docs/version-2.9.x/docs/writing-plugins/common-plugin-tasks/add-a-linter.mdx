...
If you are instead writing your own linting logic inline, you can skip Step 1. In Step 3, you will not need to use `Process`. You may find Pants's [`validate` goal implementation](https://github.com/pantsbuild/pants/blob/2.9.x/src/python/pants/backend/project_info/source_file_validator.py) helpful for how to integrate custom linting logic into Pants.
