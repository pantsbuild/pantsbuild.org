...
If unspecified, and the `lockfile` option is unset, the tool will be installed using the default lockfile shipped with Pants, which uses `flake8` version 6.1.0.
...
To instead load third-party plugins, add them to a custom resolve alongside flake8 itself, as described in https://www.pantsbuild.org/2.30/docs/python/overview/lockfiles#lockfiles-for-tools.
