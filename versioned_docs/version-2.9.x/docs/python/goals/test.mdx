...
Add [`pytest-icdiff`](https://github.com/hjwp/pytest-icdiff#usage) and [`pygments`](https://github.com/pygments/pygments) to the option `extra_requirements` for better error messages and diffs from Pytest.
:::
...
Refer to [Resources](../../using-pants/assets-and-archives.mdx) for how to include resource files in your tests by adding to the `dependencies` field.
