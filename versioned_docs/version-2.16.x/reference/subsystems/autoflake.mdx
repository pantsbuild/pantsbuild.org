...
If true, don&#x27;t use Autoflake when running `scie-pants-linux-x86_64 fmt` and `scie-pants-linux-x86_64 lint`.
...
If true, export a virtual environment with Autoflake when running `scie-pants-linux-x86_64 export`.
