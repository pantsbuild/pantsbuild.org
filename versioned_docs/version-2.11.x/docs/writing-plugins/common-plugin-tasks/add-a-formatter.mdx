...
Finally, register your new `LintTargetsRequest`/`FmtRequest` with two [`UnionRule`s](../the-rules-api/union-rules-advanced.mdx) so that Pants knows your formatter exists:
