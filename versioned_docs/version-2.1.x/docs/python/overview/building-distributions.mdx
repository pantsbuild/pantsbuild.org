...
The `pex_binary` target must either explicitly set the field `entry_point` in the form `path.to.module:my_func`, e.g. `project.app:main`; or set `entry_point` to the shorthand `:my_func` and set the `sources` field to exactly one Python file. See https://python-packaging.readthedocs.io/en/latest/command-line-scripts.html#the-console-scripts-entry-point.
