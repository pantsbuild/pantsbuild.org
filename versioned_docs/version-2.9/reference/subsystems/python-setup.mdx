...
<Option
  cli_repr={`--[no-]python-setup-macos-big-sur-compatibility`}
  env_repr='PANTS_PYTHON_SETUP_MACOS_BIG_SUR_COMPATIBILITY'
...
<Option
  cli_repr={`--python-setup-experimental-resolves-to-lockfiles="{'key1': val1, 'key2': val2, ...}"`}
  env_repr='PANTS_PYTHON_SETUP_EXPERIMENTAL_RESOLVES_TO_LOCKFILES'
  toml_repr={`[python-setup.experimental_resolves_to_lockfiles]
key1 = val1
key2 = val2
...`}
  default_repr={`{}`}
...
<Option
  cli_repr={`--[no-]python-setup-run-against-entire-lockfile`}
  env_repr='PANTS_PYTHON_SETUP_RUN_AGAINST_ENTIRE_LOCKFILE'
  toml_repr={`[python-setup]
...
<Option
  cli_repr={`--[no-]python-setup-tailor-requirements-targets`}
  env_repr='PANTS_PYTHON_SETUP_TAILOR_REQUIREMENTS_TARGETS'
  toml_repr={`[python-setup]
