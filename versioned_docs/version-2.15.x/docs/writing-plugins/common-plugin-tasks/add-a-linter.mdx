...
There are several ways for Pants to install your linter. See [Installing tools](../the-rules-api/installing-tools.mdx).
...
Options are registered through a [`Subsystem`](../the-rules-api/options-and-subsystems.mdx).
...
As described in [Rules and the Target API](../the-rules-api/rules-and-the-target-api.mdx), a `FieldSet` is a way to
