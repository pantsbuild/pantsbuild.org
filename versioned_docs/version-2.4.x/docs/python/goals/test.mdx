...
## Config files (e.g. `pytest.ini`)

Use the option `--config` in the `pytest` scope to ensure Pants includes the file in the test run. For example:
...
```text tab={"label":"pytest.ini"}
[pytest]
addopts = -s
```

:::caution Where to place the config file
Pytest does not support declaring a specific path for config files. Instead, it discovers them using [this process](https://docs.pytest.org/en/stable/customize.html#initialization-determining-rootdir-and-configfile). This means that you will likely need to place your config file in the root of your project, rather than in a directory like `build-support/`.

Alternatively, you can configure Pytest's `--rootdir` option by using Pants's `--pytest-args` option (see ["Passing arguments to Pytest"](#passing-arguments-to-pytest)), although this may mess up Pytest's output and plugin loading.
:::
