...
### `enable_stack_trampoline`

<Option
  cli_repr={`--enable-stack-trampoline=<str>`}
  env_repr='PANTS_ENABLE_STACK_TRAMPOLINE'
  toml_repr={`[GLOBAL]
enable_stack_trampoline = <str>`}
  default_repr={`None`}
>
...
<Option
  cli_repr={`--remote-provider=<RemoteProvider>`}
  env_repr='PANTS_REMOTE_PROVIDER'
  toml_repr={`[GLOBAL]
remote_provider = <RemoteProvider>`}
  one_of='reapi, experimental-file, experimental-github-actions-cache'
  default_repr={`reapi`}
>

The type of provider to use, if using a remote cache and/or remote execution, See https://www.pantsbuild.org/2.33/docs/using-pants/remote-caching-and-execution for details.
...
- `experimental-github-actions-cache`: the GitHub Actions caching service (supported schemes for URIs: `http://`, `https://`)
...
<Option
  cli_repr={`--[no-]sandboxer`}
  env_repr='PANTS_SANDBOXER'
  toml_repr={`[GLOBAL]
sandboxer = <bool>`}
  default_repr={`False`}
>

Enables use of the sandboxer process. The sandboxer materializes files into the sandbox on behalf of the main process (either pantsd or the pants client process if running without pantsd). This works around a well-known race condition when a multithreaded program writes executable files and then spawns subprocesses to execute them, which can lead to ETXTBSY errors.

This is a new feature so it is off by default. In the future, once this is stable, it will likely default to True.
...
<Option
  cli_repr={`--session-end-tasks-timeout=<float>`}
  env_repr='PANTS_SESSION_END_TASKS_TIMEOUT'
  toml_repr={`[GLOBAL]
...
Include only targets with these tags (optional &#x27;+&#x27; prefix) or without these tags (&#x27;-&#x27; prefix). See https://www.pantsbuild.org/2.33/docs/using-pants/advanced-target-selection.
...
<Option
  cli_repr={`--allow-deprecated-macos-versions="['<str>', '<str>', ...]"`}
  env_repr='PANTS_ALLOW_DEPRECATED_MACOS_VERSIONS'
  toml_repr={`[GLOBAL]
allow_deprecated_macos_versions = [
    '<str>',
    '<str>',
...
Python files to evaluate and whose symbols should be exposed to all BUILD files. See https://www.pantsbuild.org/2.33/docs/writing-plugins/macros.
...
This option cannot be overridden via environment targets, so if you need a different value than what the rest of your organization is using, override the value via an environment variable, CLI argument, or `.pants.rc` file. See https://www.pantsbuild.org/2.33/docs/using-pants/key-concepts/options.
...
<Option
  cli_repr={`--[no-]docker-execution`}
  env_repr='PANTS_DOCKER_EXECUTION'
  toml_repr={`[GLOBAL]
...
<Option
  cli_repr={`--file-downloads-max-attempts=<int>`}
  env_repr='PANTS_FILE_DOWNLOADS_MAX_ATTEMPTS'
  toml_repr={`[GLOBAL]
...
<Option
  cli_repr={`--file-downloads-retry-delay=<float>`}
  env_repr='PANTS_FILE_DOWNLOADS_RETRY_DELAY'
  toml_repr={`[GLOBAL]
file_downloads_retry_delay = <float>`}
  default_repr={`0.2`}
>

When Pants downloads files (for example, for the `http_source` source), Pants will retry the download if a &#x22;retryable&#x22; error occurs. Between each attempt, Pants will delay a random amount of time using an exponential backoff algorithm.
...
<Option
  ...
  toml_repr={`[GLOBAL.log_levels_by_target]
key1 = val1
key2 = val2
...`}
  default_repr={`{}`}
...
Note that this is currently under development and is _not_ yet useful to end users. Do not set this unless you truly know what you&#x27;re doing.
...
When set, a base directory in which to store `--pants-workdir` contents. If this option is set, the workdir will be created as a symlink into a per-workspace subdirectory.
...
pants_subprocessdir = <str>`}
  default_repr={`<buildroot>/.pants.d/pids`}
>
...
If you use the `pants` script from https://www.pantsbuild.org/2.33/docs/getting-started/installing-pants, however, changing the value in your `pants.toml` will cause the new version to be installed and run automatically.
...
<Option
  ...
  toml_repr={`[GLOBAL]
pants_workdir = <dir>`}
  default_repr={`<buildroot>/.pants.d/workdir`}
>
...
Whether or not to use nailgun to run JVM requests that are marked as supporting nailgun. Note that nailgun only works correctly on JDK &#x3C;= 17 and must be disabled manually for later versions.
...
<Option
  cli_repr={`--remote-cache-rpc-timeout-millis=<int>`}
  env_repr='PANTS_REMOTE_CACHE_RPC_TIMEOUT_MILLIS'
  toml_repr={`[GLOBAL]
...
remote_cache_warnings = <RemoteCacheWarningsBehavior>`}
  one_of='ignore, first_only, backoff, always'
...
<Option
  cli_repr={`--remote-client-certs-path=<str>`}
  env_repr='PANTS_REMOTE_CLIENT_CERTS_PATH'
  toml_repr={`[GLOBAL]
...
<Option
  cli_repr={`--remote-client-key-path=<str>`}
  env_repr='PANTS_REMOTE_CLIENT_KEY_PATH'
  toml_repr={`[GLOBAL]
...
The URI of a server/entity used as a remote execution scheduler. The supported URIs depends on the value of the `remote_provider` option.
...
<Option
  cli_repr={`--remote-execution-append-only-caches-base-path=<str>`}
  env_repr='PANTS_REMOTE_EXECUTION_APPEND_ONLY_CACHES_BASE_PATH'
  toml_repr={`[GLOBAL]
...
<Option
  ...
  toml_repr={`[GLOBAL.remote_execution_headers]
key1 = val1
key2 = val2
...`}
  default_repr={`{}`}
...
<Option
  cli_repr={`--remote-oauth-bearer-token=<str>`}
  env_repr='PANTS_REMOTE_OAUTH_BEARER_TOKEN'
  toml_repr={`[GLOBAL]
...
Recommendation: do not place a token directly in `pants.toml`, instead do one of: set the token via the environment variable (`PANTS_REMOTE_OAUTH_BEARER_TOKEN`), CLI option (`--remote-oauth-bearer-token`), or store the token in a file and set the option to `"@/path/to/token.txt"` to [read the value from that file](https://www.pantsbuild.org/2.33/docs/using-pants/key-concepts/options#reading-individual-option-values-from-files).
...
The URI of a server/entity used as a remote file store. The supported URIs depends on the value of the `remote_provider` option.
...
### `remote_store_batch_load_enabled`

<Option
  cli_repr={`--[no-]remote-store-batch-load-enabled`}
  env_repr='PANTS_REMOTE_STORE_BATCH_LOAD_ENABLED'
  toml_repr={`[GLOBAL]
...
<Option
  ...
  toml_repr={`[GLOBAL.remote_store_headers]
key1 = val1
key2 = val2
...`}
  default_repr={`{}`}
...
<Option
  cli_repr={`--remote-store-rpc-timeout-millis=<int>`}
  env_repr='PANTS_REMOTE_STORE_RPC_TIMEOUT_MILLIS'
  toml_repr={`[GLOBAL]
...
<Option
  cli_repr={`--unmatched-build-file-globs=<GlobMatchErrorBehavior>`}
...
<Option
  cli_repr={`--unmatched-cli-globs=<GlobMatchErrorBehavior>`}
